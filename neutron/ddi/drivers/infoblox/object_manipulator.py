...
class InfobloxObjectManipulator(object):
    ...
    def update_network_options(self, ib_network):
    ...
    def create_dns_zone(self, dns_view, dns_zone_fqdn, member=None,
                        zone_format=None, ns_group=None):
        # TODO support IPv6
        dns_zone_data = {'fqdn': dns_zone_fqdn,
                         'view': dns_view}
        additional_create_kwargs = {}
        if member:
            grid_primary = [{'name': member.name, '_struct': 'memberserver'}]
        ...
        try:
            ...
            LOG.warning(_('Unable to create DNS zone %s for %s'),
                        dns_zone_fqdn, dns_view)
    ...
    def _next_available_ip(net_view_name, first_ip, last_ip):
        return ('func:nextavailableip:'
                '%(first_ip)s-%(last_ip)s,%(net_view_name)s') % locals()
    ...
    def _create_infoblox_object(self, obj_type, payload,
        ...
        if check_if_exists:
            ...
            if ib_object:
                LOG.info(_("Infoblox %s already exists: %s"),
                         obj_type, ib_object)
        ...
        if not ib_object:
            ...
            ib_object = self.connector.create_object(obj_type, payload,
                                                     return_fields)
            LOG.info(_("Infoblox %s was created: %s"), obj_type, ib_object)
    ...
    def _update_infoblox_object(self, obj_type, payload, update_kwargs):
        ib_object_ref = None
        warn_msg = _('Infoblox %s will not be updated because'
                     ' it cannot be found: %s')
        try:
    ...
    def _delete_infoblox_object(self, obj_type, payload):
        ib_object_ref = None
        warn_msg = _('Infoblox %s will not be deleted because'
                     ' it cannot be found: %s')
        try:
            ib_object_ref = self._get_infoblox_object_or_none(obj_type,
                                                              payload)
